...
首页插图细节：五子棋棋子沿居中的五个网格交点排列；路墙棋初始棋子对角对称、墙在中央，演示路径沿左侧绕墙。扑克牌中央点数缩小以留出角标空间。宝石聚焦效果叠加双层光环、放射光和六枚星芒；普通模式分阶段展开，减少动画模式显示静态光晕，触屏不触发悬停效果。验收时同时检查静态坐标、动画终态和移出后恢复。
...
规则测试固定完整卡池摘要、贵族清单、2/3/4 人设置，并检查完整模拟对局中的宝石与卡牌守恒。支付界面测试从完整卡池构建固定市场与剩余牌堆，校验卡牌唯一性，避免随机发牌与指定卡牌重复造成偶发失败。联机测试覆盖每种必选阶段的重连与文件数据库重启，以及状态和回执插入失败时的事务回滚。迁移测试使用冻结的 v3 SQL 文件创建带旧对局、成员、消息和回执的数据库，验证升级、重复启动、失败回滚、外键与完整性。
...
宝石采用多层 SVG 切面渐变、边缘高光和接触阴影；首页与供应区使用精细版本，小图标保留清晰轮廓，黄金呈现金属切面。个人持有区按颜色等宽并排显示「永久」「临时」，两栏图标、数字尺寸与颜色一致；黄金永久栏显示「—」。永久奖励用于购买折扣，十枚持有上限只统计临时宝石与黄金。
...
宝石与一键开局优化另在 375px、768px、872px、1440px 验收首页、供应和持有区：无横向溢出，同色永久／临时保持并排、同字号和同图标大小；覆盖永久 3／临时 2、零值、黄金特殊显示及键盘直接开局。新增界面测试覆盖等待原因、普通成员取消准备、房主交接和提交锁；真实客户端测试验证房主未预先准备、离线拒绝、并发与过期请求、重开，以及回执写入异常时房主准备状态一并回滚。

- 至少两位、全员在线且其他成员均已准备后，房主点击唯一的“开始游戏”按钮，通过 `room:start` 开局，无需提前准备。房主座位显示“房主 · 等待开局”，普通成员仍可准备或取消；条件不满足时按钮显示对应等待原因。新局重新洗牌并由服务端随机首家，按座位顺序循环；开局后不接纳新玩家。
- 认输、主动离开、断线超过 60 秒或重启恢复超过十分钟，中止且不计胜负，显示原因与当时分数，不生成赢家。无合法主行动时才可跳过，整轮全员无法行动则中止。这是明确标注在规则帮助中的线上补充约定；默认无回合倒计时。
- 结束或中止后，房主通过 `room:reopen` 保留邀请码，释放离线座位并清空准备和旧对局，由其他成员重新准备、房主点击开始。房主离开、等待区房主断线，或中止后房主仍离线时，交给最早入房的在线成员。离开当前房间后才能加入其他房间。
- `room:start` / `room:reopen` 均携带 `{ commandId, roomId, expectedVersion }`，权限、阶段和其他成员准备状态在事务内检查；房主点击时的准备状态、初始化对局及回执原子保存，失败全部回滚。重复命令返回原回执，过期或篡改请求不再洗牌、重开；命令和 `game:action` 共用回执表，编号不能跨用途复用。
...
首页展示：四款游戏在大于 620px 时统一为 2×2，手机单列。璀璨宝石支持悬停／键盘聚焦的舒展与高光动效（触屏及减少动画模式保持静态）；24 点展示黑桃、红桃、梅花、方块六点牌，汇聚后在圆形徽章中心显示 24。24 点沿用原有桌面悬停／聚焦动效，不受璀璨宝石的减少动画样式影响；触屏仍采用静态展示。前端检查使用 pnpm verify:web，浏览器验收覆盖 375、620、621、698、768、872、1440px，并在系统开启减少动画时检查 24 点仍能汇聚显现。
