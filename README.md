...
packages/game-core       四款游戏的规则引擎
packages/protocol        客户端与服务端共享的事件协议
docs/                    验证流程与项目文档
render.yaml              Render 单实例部署配置
```

`packages/game-core` 已包含标准德州扑克规则引擎：2–4 人、1000 虚拟筹码、10/20 盲注、公共牌、下注和主池/边池结算；联机房间与牌桌入口正在接入中。
...
完整门禁覆盖 lint、typecheck、单元测试、真实 HTTP/Socket.IO 集成测试、生产构建和负载冒烟。集成测试覆盖四款游戏、房间边界、重复与过期动作、断线恢复、重启恢复、房间消息、房主交接和璀璨宝石重开；负载冒烟覆盖 50 个连接和 25 个活跃房间。
